...
def reset():
    ...
    textgrill.config(state=DISABLED)
...
def receipt():
    ...
    if costoffoodvar.get() != '' or costofcakesvar.get() != '' or costofdrinksvar.get() != '':
        ...
        if e_dosa.get()!='0':
            textReceipt.insert(END,f'Dosa\t\t\t{int(e_dosa.get())*50}\n\n')

        if e_fish.get()!='0':
            textReceipt.insert(END,f'Fish\t\t\t{int(e_fish.get())*100}\n\n')

        if e_piryani.get() != '0':
            textReceipt.insert(END, f'Piryani:\t\t\t{int(e_piryani.get()) * 199}\n\n')

        if e_grill.get() != '0':
            textReceipt.insert(END, f'Grill:\t\t\t{int(e_grill.get()) * 499}\n\n')

        if e_parotta.get() != '0':
            textReceipt.insert(END, f'Paneer:\t\t\t{int(e_parotta.get()) * 30}\n\n')

        if e_pizza.get() != '0':
            textReceipt.insert(END, f'Pizza:\t\t\t{int(e_pizza.get()) * 199}\n\n')
        ...
        if e_tea.get() != '0':
            textReceipt.insert(END, f'Tea:\t\t\t{int(e_tea.get()) * 30}\n\n')

        if e_milkshake.get() != '0':
            textReceipt.insert(END, f'Milkshake:\t\t\t{int(e_milkshake.get()) * 99}\n\n')

        if e_jigarthanda.get() != '0':
            textReceipt.insert(END, f'Jigarthanda:\t\t\t{int(e_jigarthanda.get()) * 60}\n\n')
        ...
        if e_wine.get() != '0':
            textReceipt.insert(END, f'Wine:\t\t\t{int(e_wine.get()) * 199}\n\n')
...
def totalcost():
    ...
    if var1.get() != 0 or var2.get() != 0 or var3.get() != 0 or var4.get() != 0 or var5.get() != 0 or \
        ...
        item1=int(e_vada.get())
        item2=int(e_dosa.get())
        item3=int(e_fish.get())
        item4 = int(e_grill.get())
        item5 = int(e_pizza.get())
        item6 = int(e_piryani.get())
        ...
        priceofFood=(item1*10)+(item2*50)+(item3*100)+(item4*499)+ (item5*199) + (item6 * 199) + (item7 * 120) \
                    + (item8 * 30) + (item9 * 120)

        priceofDrinks=(item10*50)+(item11*40)+ (item12 * 80) + (item13 * 30) + (item14 * 99) + (item15 * 60) \
                      + (item16 * 20) + (item17 * 50) + (item18 * 199)
...
def grill():
    if var4.get() == 1:
        textgrill.config(state=NORMAL)
        textgrill.focus()
        textgrill.delete(0, END)
    elif var4.get() == 0:
        textgrill.config(state=DISABLED)
        e_gril.set('0')
...
def piryani():
    if var6.get() == 1:
        textpiryani.config(state=NORMAL)
        textpiryani.focus()
        textpiryani.delete(0, END)
    elif var6.get() == 0:
        textpiryani.config(state=DISABLED)
        e_piryani.set('0')
...
def jigarthanda():
    if var15.get() == 1:
        textjigarthanda.config(state=NORMAL)
        textjigarthanda.focus()
        textjigarthanda.delete(0, END)
    elif var15.get() == 0:
        textjigarthanda.config(state=DISABLED)
        e_jigarthanda.set('0')
...
root.title('Zara Restaurant created by Faizan Khan')
...
labelTitle=Label(topFrame,text=' Zara Restaurant',font=('arial',30,'bold'),fg='yellow',bd=9,
                 bg='red4',width=51)
...
vada=Checkbutton(foodFrame,text='Vada',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var1
                 ,command=vada)
vada.grid(row=0,column=0,sticky=W)

dosa=Checkbutton(foodFrame,text='Dosa',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var2
                 ,command=dosa)
...
grill=Checkbutton(foodFrame,text='Grill',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var4
                  ,command=grill)
grill.grid(row=3,column=0,sticky=W)
...
piryani=Checkbutton(foodFrame,text='Piryani',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var6
                   ,command=piryani)
piryani.grid(row=5,column=0,sticky=W)
...
textgrill = Entry(foodFrame, font=('arial', 18, 'bold'), bd=7, width=6, state=DISABLED, textvariable=e_grill)
textgrill.grid(row=3, column=1)
...
textpiryani = Entry(foodFrame, font=('arial', 18, 'bold'), bd=7, width=6, state=DISABLED, textvariable=e_piryani)
textpiryani.grid(row=5, column=1)
...
tea=Checkbutton(drinksFrame,text='tea',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var13
                     ,command=tea)
...
jigarthanda=Checkbutton(drinksFrame,text='Jigarthanda',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var15
                     ,command=jigarthanda)
jigarthanda.grid(row=5,column=0,sticky=W)
...
wine=Checkbutton(drinksFrame,text='Wine',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var18
                       ,command=wine)
...
textjigarthanda = Entry(drinksFrame, font=('arial', 18, 'bold'), bd=7, width=6, state=DISABLED, textvariable=e_jigarthanda)
textjigarthanda.grid(row=5, column=1)
